...
export function OrderBook({ basePrice, baseSymbol, quoteSymbol }: OrderBookProps) {
    ...
    // Kalkulasi Dominasi Bid/Ask untuk indikator persentase
    ...
    const askPercent = totalVol > 0 ? Math.round((totalAskVol / totalVol) * 100) : 50;
    ...
    const fmtTotal = (t: number) =>
        t.toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 });

    // Handler Tooltip
    const handleMouseEnter = (e: React.MouseEvent, order: OrderEntry) => {
        const rect = e.currentTarget.getBoundingClientRect();
        setHoveredOrder(order);
        setTooltipPos({
            x: rect.left, // Posisi X dimulai dari kiri elemen
            y: rect.top + (rect.height / 2) // Posisi Y di tengah vertikal elemen
        });
    };
    ...
    return (
        <div className="relative flex flex-col h-full bg-[#0d0e14] border border-white/5 overflow-hidden font-sans">
            ...
            {/* Tooltip Component - Minimalist, Left Aligned, No Radius */}
            {hoveredOrder && (
                <div
                    className="fixed z-50 pointer-events-none bg-[#0d0e14] border border-white/10 shadow-2xl p-2 min-w-[140px]"
                    style={{
                        left: tooltipPos.x - 150, // Geser ke kiri elemen (lebar tooltip approx 140 + gap 10)
                        top: tooltipPos.y - 20, // Sedikit offset ke atas agar center
                    }}
                >
                    <div className="space-y-1 text-[10px]">
                        ...
                        <div className="text-gray-200 font-mono text-xs truncate">
                            {fmtTotal(hoveredOrder.total)} <span className="text-gray-600">{quoteSymbol}</span>
                        </div>
                        <div className="h-px bg-white/5 my-1" />
                        <div className="flex justify-between text-gray-500">
                            <span>Price</span>
                            <span className={cn(
                                hoveredOrder.price >= basePrice ? "text-green-500" : "text-red-500"
                            )}>
            ...
            <div className="px-3 pt-2 pb-1 border-b border-white/5">
                <div className="grid grid-cols-3 text-[10px] text-gray-600 mb-2">
                    ...
                    <span className="text-right">Total ({quoteSymbol})</span>
                </div>

                {/* Bid/Ask Dominance Indicator */}
                <div className="flex items-center gap-2 text-[10px]">
                    <span className={cn("w-8 text-right font-medium", bidPercent > 50 ? "text-green-400" : "text-gray-500")}>
                        {bidPercent}%
                    </span>
                    ...
                    <span className={cn("w-8 font-medium", askPercent > 50 ? "text-red-400" : "text-gray-500")}>
                        {askPercent}%
                    </span>
                </div>
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto flex flex-col justify-end">
                        {asks.map((order, i) => (
                            ...
                            >
                                ...
                                />
                                <span className="text-red-400 tabular-nums z-10">{fmtPrice(order.price)}</span>
                                <span className="text-right text-gray-400 tabular-nums z-10">{fmtSize(order.size)}</span>
                                <span className="text-right text-gray-500 tabular-nums z-10">{fmtTotal(order.total)}</span>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {view === "both" && (
                    <div className="flex items-center justify-between px-3 py-2 border-y border-white/5 bg-white/2">
                        <span className={cn(
                            ...
                            (bids[0]?.price ?? 0) > (asks[asks.length - 1]?.price ?? 0)
                                ? "text-green-400"
                                : "text-red-400"
                        )}>
                        ...
                        {bids.map((order, i) => (
                            ...
                            >
                                <div
                                    ...
                                    style={{ width: `${(order.total / maxTotal) * 100}%` }}
                                />
                                <span className="text-green-400 tabular-nums z-10">{fmtPrice(order.price)}</span>
                                <span className="text-right text-gray-400 tabular-nums z-10">{fmtSize(order.size)}</span>
                                <span className="text-right text-gray-500 tabular-nums z-10">{fmtTotal(order.total)}</span>
                            </div>
                        ))}
